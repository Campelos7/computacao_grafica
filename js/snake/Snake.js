...
export class Snake {
  ...
  render(alpha) {
    ...
    for (let i = 0; i < this.segments.length; i++) {
      ...
      if (!mesh) continue;
      const wx = THREE.MathUtils.lerp(gridToWorldX(prev.x), gridToWorldX(target.x), alpha);
      const wz = THREE.MathUtils.lerp(gridToWorldZ(prev.z), gridToWorldZ(target.z), alpha);
      mesh.position.set(
        wx,
        SNAKE_RENDER_Y,
        wz,
    ...
    if (this.tailGroup && this.segments.length >= 2) {
      ...
      this.tailGroup.position.set(
        THREE.MathUtils.lerp(gridToWorldX(prevSeg.x), gridToWorldX(lastSeg.x), alpha),
        0,
        THREE.MathUtils.lerp(gridToWorldZ(prevSeg.z), gridToWorldZ(lastSeg.z), alpha),
      );
  ...
  emitTrail() {
    ...
    p.position.set(gridToWorldX(this.segments[0].x), 0.15, gridToWorldZ(this.segments[0].z));
    p.userData.life = 1.0;
  ...
  explode() {
    ...
    for (let s = 0; s < this.segments.length; s++) {
      ...
      for (let i = 0; i < count; i++) {
        ...
        p.position.set(gridToWorldX(seg.x), SNAKE_RENDER_Y, gridToWorldZ(seg.z));
  ...
  get headWorldPosition() {
    const head = this.segments?.[0];
    if (!head) return new THREE.Vector3(0, SNAKE_RENDER_Y, 0);
    return new THREE.Vector3(gridToWorldX(head.x), SNAKE_RENDER_Y, gridToWorldZ(head.z));
  }

  getHeadWorldInterpolated(alpha) {
    const target = this.segments?.[0];
    if (!target) return new THREE.Vector3(0, SNAKE_RENDER_Y, 0);
    const prev = this.previousSegments?.[0] || target;
    return new THREE.Vector3(
      THREE.MathUtils.lerp(gridToWorldX(prev.x), gridToWorldX(target.x), alpha),
      SNAKE_RENDER_Y,
      THREE.MathUtils.lerp(gridToWorldZ(prev.z), gridToWorldZ(target.z), alpha),
    );
  }
}
